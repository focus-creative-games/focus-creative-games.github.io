...
### 安装HybridCLR步骤： 
- 进入HybridCLRData目录
- 酌情修改 init_local_il2cpp_data.bat(或.sh)文件中两个环境变量
    - 设置 IL2CPP_BRANCH。如果你的Unity版本是2020系列，使用2020.3.33；如果2021系列，使用2021.3.1。千万**不要使用你自己当前的Unity小版本**，因为不存在对应的分支。`set IL2CPP_BRANCH=2020.3.33` 或者 `set IL2CPP_BRANCH=2021.3.1`
    - 设置IL2CPP_PATH。设置以上一个环境变量即2020.3.33或2021.3.1版本对应的il2cpp目录的安装位置。例如2020.3.33版本，你装了2020.3.33f1则，Win下为 `
IL2CPP_PATH='C:\Program Files\Unity\Hub\Editor\2020.3.33f1\Editor\Data\il2cpp' `, Mac下为 `
...
注意！！！新手最容易犯的错误是把IL2CPP_BRANCH和IL2PP_PATH指向了它自己当前使用的Unity版本！！！必须是2020.3.33或2021.3.1！！！

务必检查命令行的运行过程，确保没有错误。遇到错误，请先用自己对命令行的了解去解决它！！！
