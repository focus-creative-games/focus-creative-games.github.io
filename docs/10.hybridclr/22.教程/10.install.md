...
由于同步等原因，openupm的最新版本经常落后于github上的最新版本。如果未能找到最新版本，请使用第一种安装方式。
...
点击菜单 `HybridCLR/Installer...`，弹出安装界面。如果安装界面没有错误或者警告，则说明il2cpp路径设置正常，否则需要你手动选择正确的il2cpp目录(类似`C:\Program Files\Unity\Hub\Editor\2020.3.33f1\Editor\Data\il2cpp`)。点击`install`按钮完成安装。
