...
目前hybridclr_unity package有多个分支，其中main分支为开发分支，1.0为稳定分支。由于当前main分支也已经稳定，而且工作流更完善，新接入的项目强烈推荐直接安装main分支最新版本或 `>= 2.0.1` 的版本。
...
由于Unity自身原因，WebGL目标打包不支持本地安装，必须全局安装。需要你手动替换libil2cpp目录，并且 HybriCLRSettings中设置`useGlobalIl2cpp=true` 。每次`Generate/All`之后必须重新将`{project}/HyridCLRData/LocalIl2CppData-{platform}/il2cpp`目录复制到Editor安装目录。
