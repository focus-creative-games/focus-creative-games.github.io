...
由于Unity自身原因，WebGL目标打包不支持本地安装，必须全局安装。需要你手动替换libil2cpp目录，并且 HybriCLRSettings中设置`useGlobalIl2cpp=true` 。每次`Generate/xxx`之后必须重新将`{project}/HyridCLRData/LocalIl2CppData-{platform}/il2cpp/libil2cpp`目录替换Editor安装目录下的相应目录。
