...
- AOT泛型，需要在AOT中已经有相应的共享泛型的代码存在或者使用补充元数据技术。具体见[AOT泛型限制及原理介绍](/hybridclr/generic_limit/)。
- 对于返回**struct类型**的async实现， 需要做上一条相似的AOT泛型处理。具体见[AOT泛型限制及原理介绍](/hybridclr/generic_limit/)。
