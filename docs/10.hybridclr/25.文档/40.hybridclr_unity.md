...
- `HomologousImageMode::SuperSet` 模式，即补充的dll是打包时裁剪后的dll的超集，包含了裁剪dll的所有元数据。一个最简单易得的超集dll为原始aot dll，这也是推荐使用的超集dll。原始UnityEngine相关 aot dll在Unity安装目录的PlayBackEngines子目录下，原始的.net核心aot dll如mscorlib.dll在Unity安装目录的 unityaot 目录下。如Win下的Win64目标时，mscorlib.dll在 `{editor}/Editor/Data/MonoBleedingEdge/lib/mono/unityaot`，UnityEngine.CoreModule.dll 在 `{editor}/Editor/Data/Playbackengines/windowsstandalonesupport/Variations/il2cpp/Managed`。其他平台的类似。
