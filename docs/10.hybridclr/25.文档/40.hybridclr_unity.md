...
- `HomologousImageMode::SuperSet` 模式，即补充的dll是打包时裁剪后的dll的超集，包含了裁剪dll的所有元数据。一个最简单易得的超集dll为原始aot dll，这也是推荐使用的超集dll。原始aot dll在Unity安装目录的PlayBackEngines子目录下，如Win下的Win64目标的aot dll在 `{editor}/Editor/Data/Playbackengines/windowsstandalonesupport/Variations/il2cpp/Managed`。其他的类似。

#### RuntimeApi

底层的操作HybridCLR的工具类。比较常用的是`public static unsafe LoadImageErrorCode LoadMetadataForAOTAssembly(byte[] dllBytes)`函数，用于加载补充元数据assembly。
