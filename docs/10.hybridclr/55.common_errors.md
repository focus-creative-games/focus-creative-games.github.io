...
hybridclr_unity版本过低。请升级到0.11.0及更高版本。
...
这是.net 7发布后最新版本vs改动打破了一些向后兼容性引起。你可以回退版本或者使用2019之类的版本。

简单一点的解决办法是直接在 Player Settings里的 `Additional Compiler Arguments` 列表中新增一项

 `-D _SILENCE_STDEXT_HASH_DEPRECATION_WARNINGS` 。

一种稍微麻烦的解决办法是修改 `il2cpp/external/google/sparsehash/internal/sparseconfig.h`，新增 `#define _SILENCE_STDEXT_HASH_DEPRECATION_WARNINGS` 即可。可参照下图修改。
...
hybridclr依赖版本宏来同时支持2019-2021的il2cpp。hybridclr代码中版本宏默认是2020版本，如果你使用的不是2020，则会因为版本宏不正确而发生编译错误。你必须将hybridclr_unity更新了0.10.x版本，再运行  `HybridCLR/generate/il2cppdef` 来生成正确的版本宏。
