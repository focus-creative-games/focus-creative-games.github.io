...
因为 你使用的是原始libil2cpp.a或者过时的libil2cpp.a。请根据 [build iOS libil2cpp.a](/hybridclr/ios/) 文档编译最新的。然后替换
xcode项目中的libil2cpp.a文件
...
HybridCLR提供了快捷的自动生成工具，运行菜单命令 `HybridCLR/GenrateLinkXml` 根据热更新dll生成 link.xml
...
具体操作请看[AOT泛型限制及原理介绍](/hybridclr/generic_limit/) 文档。
...
- 如果target为iOS，需要重新编译libil2cpp.a，并且更新xcode中相应文件

### MethodAccessException 错误

在player settings里关闭增量式gc
