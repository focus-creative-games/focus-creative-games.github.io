...
### 运行 `HybridCLR/generate/xxx` 时发生 `Exception: resolve assembly: mscorlib fail`

如果你使用了1.1.6及更高版本的package，生成桥接函数需要依赖裁减后的AOT dll。因此你需要先 `generate/linkxml`，再build或者导出
...
如果你使用Unity 2021及以上版本，并且WebGL平台，这是因为2021版本的WebGL不支持资源上挂载脚本。

如果是其他版本及平台，由于Unity的资源管理的实现机制，资源必须打包为AssetBundle才能正常恢复热更新脚本，放到Resource下不行。
