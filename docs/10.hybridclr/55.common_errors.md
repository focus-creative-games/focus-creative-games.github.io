...
```
问题出现在comparer.Compare 上，它的签名是 T Compare(T, T)，以SortedDictionary<string, string>为例，它的T是KeyValuePaire<string, string>, 那么Compare的签名就是int(KeyValuePaire<string, string>, KeyValuePaire<string, string>)。于是添加Func<KeyValuePaire<string, string>，KeyValuePaire<string, string> 这个delegate到自定义桥接函数列表。

具体添加什么，是要看 SortedDictionary<T, V> 的泛型参数。 添加 Func<SortedDictionary<K, V>, SortedDictionary<K, V>, int>
