...
- 对于2020.3.16-2020.3.20版本，需要额外安装2020.3.21+版本，因为这些版本的il2cpp文件与HybridCLR不兼容，但可以使用兼容版本(即2020.3.21+)的il2cpp。**注意！1.1.7版本起installer不再支持从其他版本安装il2cpp相关文件，你只能自己按照原理手动安装。**
...
- `LoadImageErrorCode UseDifferentialHybridAOTAssembly(string assemblyName)` 当差分混合执行assembly没发生变化时，可以选择完全使用原始AOT assembly。**注意！1.1.7版本移除了此函数。**
- `LoadImageErrorCode LoadDifferentialHybridAssembly(byte[] dllBytes, byte[] optionBytes)` 加载差分混合执行assembly。**注意！1.1.7版本移除了此函数。**
