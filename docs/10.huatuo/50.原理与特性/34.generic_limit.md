...
注意，当前要求补充的dll与打包时裁剪后的dll精确一致，因此必须使用build过程中生成的裁剪后的dll，则不能直接复制原始dll，这个限制将来可能会去掉。这些dll可以在目录 `{project}/Temp/StagingArea/Data/Managed` 下找到，但是unity在打包完成之后会移动或删除这些文件，所以获取这些文件需要点小技巧，在[huatuo_trial](https://github.com/focus-creative-games/huatuo_trial)项目中通过实现`IPostBuildPlayerScriptDLLs`接口拷贝了裁剪后的dll。
...
{
    // 可以加载任意aot assembly的对应的dll。但要求dll必须与unity build过程中生成的裁剪后的dll一致，
    // 而不能直接使用原始dll。
