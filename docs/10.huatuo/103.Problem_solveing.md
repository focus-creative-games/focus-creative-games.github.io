...
- huatuo是否正确安装，huatuo_il2cpp是否为正确的分支，huatuo是否为最新版本
- Project Settings中Configuration是否设置正确，是否关闭增量GC
- 请检查是否打包了32位或Android中armv7的target，目前huatuo尚不支持
...
- 是否已经关闭了增量GC选项
- huatuo解释器不支持extern函数，请检查是否将其放到AOT部分
...
请确定你遇到了哪类问题，并根据引导仔细阅读原理及常见错误处理文档（查看[常见错误文档](/huatuo/common_errors/)），并按照文档提示进行检查。
...
请查看热更新部分是否使用了extern函数，huatuo解释器不支持extern函数，请将其放入AOT部分
...
不能直接从ScriptAssemblies里拷贝dll，因为可能会包含不正确的宏开关，导致引用了Editor或者其他代码。必须使用脚本编译。可参见 HuatuoEditorHelper (opens new window)中 CompileDll 代码。
